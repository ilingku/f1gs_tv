...
function initializePageContent() {
    ...
    // 设置页面标题
    document.title = currentVideoTitle + ' - F1.GS播放器';
    document.getElementById('videoTitle').textContent = currentVideoTitle;

    // 初始化播放器
    if (videoUrl) {
		if(isHtmlUrl(videoUrl)==true){
		iframe_player(videoUrl);
		} else {
        initPlayer(videoUrl);
		}
    } else {
...
function initPlayer(videoUrl) {
    ...
    art = new Artplayer({
		...
		controls: [{
			position: "left",
            index: 13,
            html: '<i class="art-icon flex" onclick="playNextEpisode()"><svg width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M6 18l8.5-6L6 6v12zM16 6v12h2V6h-2z" fill="currentColor"/></svg></i>',
            tooltip: "播放下一集",
        }]
    });
...
function isHtmlUrl(videoUrl) {
    // 检查输入是否为字符串
    if (typeof videoUrl !== 'string') {
        return false;
    }
    
    // 去除可能的空格并转换为小写（可选，根据需求决定是否区分大小写）
    const trimmedUrl = videoUrl.trim().toLowerCase();
    
    // 检查是否以 .html 结尾
    return trimmedUrl.endsWith('.html');
}

function iframe_player(videoUrl){
            // 获取id为player的div元素
            ...
            if (playerDiv) {
                ...
                // 设置iframe的属性
                iframe.src = atob("Ly92MS5mMS5ncy9qaWV4aS8/dXJsPQ==")+videoUrl;
                iframe.width = '100%';
                iframe.height = '500px';
                ...
                // 清空player div的现有内容（可选）
                playerDiv.innerHTML = '';
                
                // 将iframe添加到player div中
                playerDiv.appendChild(iframe);
            } else {
                console.error('未找到id为"player"的div元素');
            }
}