// 全局变量
let selectedAPIs = JSON.parse(localStorage.getItem('selectedAPIs') || '["dyttzy","bfzy", "tyyszy", "hhzy","wwzy"]'); // 默认选中资源
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (!localStorage.getItem('hasInitializedDefaults')) {
        // 默认选中资源
        selectedAPIs = ["dyttzy", "bfzy", "tyyszy", "hhzy","wwzy"];
...
function resetSearchArea() {
    ...
    try {
        ...
        // 更新页面标题
        document.title = `F1.GS - 免费在线视频搜索与观看平台`;
    } catch (e) {
                ...
                <div class="card-hover bg-[#111] rounded-lg overflow-hidden cursor-pointer transition-all hover:scale-[1.02] h-full shadow-sm hover:shadow-md" 
                    ...
                    <div class="flex h-full">
                        ...
                        <div class="relative flex-shrink-0 search-card-img-container">
                            <img src="${ilingku_getpic(item.vod_pic)}" alt="${safeName}" 
                                 class="h-full w-full object-cover transition-transform hover:scale-110" 
                                 onerror="this.onerror=null; this.src='https://img2.baidu.com/it/u=714396849,3604975761&fm=253&fmt=auto&app=138&f=JPEG'; this.classList.add('object-contain');" 
                                 loading="lazy">
...
async function importConfigFromUrl() {
    ...
    document.getElementById('confirmUrlImport').addEventListener('click', async () => {
        ...
        try {
            ...
            if (config.name !== 'F1.GS-Settings') throw '配置文件格式不正确';
...
async function exportConfig() {
    ...
    config['name'] = 'F1.GS-Settings';  // 配置文件名，用于校验
    ...
    // 将配置数据保存为 JSON 文件
    saveStringAsFile(JSON.stringify(config), 'F1.GS-Settings_' + times + '.json');
}
...
function ilingku_getpic(originalUrl) {
    let processedUrl = originalUrl.replace(/^https?:\/\//i, '');
    let encodedUrl = encodeURIComponent(processedUrl);
    const baseUrl = atob("aHR0cHM6Ly9naW1nMy5iYWlkdS5jb20vZ2ltZy9hcHA9MjAyOCZmbXQ9YXV0byZzcmM9");
